...
const int sq80Parameters[32] = {
  ...
  2,  6,  10, 14, 16, 33, 46, 53,
  // Pots 17-24: OSC1 Fine | OSC2 Fine | OSC3 Fine | DCA3 Vol | LFO1 Dly | LFO3 Freq | ENV1 Rel | ENV2 Rel
...
int translateToEnsoniqParam(int internalID) {
  switch(internalID) {
    ...
    // Misc
    case 23: return 30; // PATCH VOLUME (DCA 4 VOL)
...
// --- MIDI MERGE HELPER ---
// Drains all pending bytes from Serial1 RX (MIDI IN) to Serial1 TX (MIDI OUT).
// Called in multiple places — top of loop(), inside the scan loop, and before
// each NRPN send — so the 64-byte RX hardware buffer never has time to overflow
// even during slow I2C OLED updates or back-to-back parameter changes.
// All channels are passed through unconditionally; this is a transparent merge.
inline void flushMIDIMerge() {
...
void loop() {
  ...
  // MIDI merge: drain RX → TX at the top of every loop() iteration.
  // Also called inside the scan loop and before each NRPN send.
  flushMIDIMerge();
  ...
  for (int channel = 0; channel < 16; channel++) {
    // Service the MIDI merge on every channel iteration so incoming bytes
    // are forwarded even during pot scans and OLED updates.
    flushMIDIMerge();
  ...
  if (abs(currentRawValue - lastPotState[potID]) > hysteresis) {
    ...
    if (shiftActive) {
      switch(potID) {
        ...
        // --- DCA / FILTER / MISC (Col 4 shifted) ---
        case 3:  paramID = 18; break; // Pot 4:  DCA 1 Vol      -> Filter Cutoff
        case 11: paramID = 19; break; // Pot 12: DCA 2 Vol      -> Filter Resonance
        case 19: paramID = 82; break; // Pot 20: DCA 3 Vol      -> Filter Mod Amount 3
    ...
    if (paramID == 18) {
      ...
      if (glitchMode) {
        // Glitch mode extends beyond the standard wavetable.
        // Hard ceiling is 127 — MIDI data bytes are 7-bit only.
        // Values >= 128 set the high bit and are interpreted as status bytes,
        // corrupting the NRPN message. Hidden waves live in the 75-127 range
        // (SQ-80) or 32-127 range (ESQ-1) — both fully reachable at 0-127.
        ensoniqValue = map(currentRawValue, 0, 1023, 0, 127);
      } else {
    ...
    // Drain the merge buffer immediately before sending our own NRPN bytes,
    // so incoming MIDI is forwarded before we occupy the TX buffer.
    flushMIDIMerge();
